...
mod tests {
    ...
    /// libfdk-aac がロードできることを前提として要求する
    ///
    /// AAC のデコードテストは libfdk-aac の動的ロードが前提。未導入の環境で
    /// 「検証されないまま成功」させず、明示的に失敗させて前提の欠落を可視化する。
    #[cfg(all(target_os = "linux", feature = "fdk-aac"))]
    fn require_fdk_aac_lib() {
        shiguredo_fdk_aac::FdkAacLibrary::load("libfdk-aac.so.2").expect(
            "AAC デコードテストには libfdk-aac.so.2 が必要です。Linux では libfdk-aac-dev を導入してください",
        );
    }
