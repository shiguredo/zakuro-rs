...
あわせて `--input-mp4` 系の姉妹テスト 2 件も強化する。この 2 件は排他文言そのものではなく含まれるオプション名だけを assert している。`--input-mp4` は他の複数の排他エラーや noargs の値エラーにも現れる語なので、到達したエラーが入れ替わっても assert が成立しうる。排他文言そのものを検証して、どの検証に到達したのか読み取れる形にする。
...
3. assert は 5 キー共通で排他文言の逐語を含むことにする。`vp8` 以外のキーを指定しても文言は `--vp8-encoder 等` のまま (実際に指定したキー名は文言に出ない) であり、キー名での assert は行わない
4. 姉妹テスト 2 件も実際に返る排他文言の逐語に揃え、他の排他エラーと混同しても pass しない形にする。`parse_args_from_argv_rejects_input_mp4_with_openh264` は `--input-mp4 と --openh264 は同時に指定できません` (`parse_args_from_argv()` 由来)、`parse_args_from_argv_rejects_input_mp4_with_input_wav` は `--input-mp4 と --input-wav は同時に指定できません` (`parse_instance_args()` 由来) を検証する
5. 検証形は既存の `assert!(msg.contains(排他文言), "...: {msg}")` を維持する。排他エラーは `AppError::Message` として返り `format!("{err}")` の先頭に `AppError::Message: ` が付くため、`assert_eq!` による完全一致は使わない

## 完了条件

- 5 キーそれぞれの反復で排他文言を逐語 assert していること。`parse_instance_args()` の排他検証の条件を 1 つでも壊すと当該テストが fail すること (エンコーダー実装指定との排他検証を一時的に無効化して fail を確認し、確認後は元に戻してコミットしない)
- `--input-mp4` 系の排他検証テストが、エンコーダー実装指定・`--openh264`・`--input-wav` のいずれについても実測の排他文言を逐語含むことを検証していること (`contains` による逐語一致であり完全一致ではない)
