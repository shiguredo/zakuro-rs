...
- `Makefile` には `fuzzing` / `fuzzing-parallel` / `fuzzing-list` の target が並ぶが、`fuzz/` が無く `cargo fuzz` も未導入のため実行できない (削除は `issues/0067` で扱う)
- 公開 API が無いため、外部 fuzz クレートから対象関数を呼べない (**前提: `issues/0071`**)
