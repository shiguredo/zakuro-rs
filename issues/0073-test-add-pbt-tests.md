...
- `Makefile` には `pbt` と `pbt-with-cover` が並ぶが、`pbt` パッケージは存在しないため `cargo llvm-cov -p pbt --tests` は失敗する (削除は `issues/0067` で扱う)
- 公開 API が無いため、外部テストクレートから検証対象を呼べない (**前提: `issues/0071`**)
...
2. 最初に検証するプロパティは、ラウンドトリップと冪等性が自然に定義できる変換に絞る
   ...
   - 引数解析の妥当性: `src/args.rs` の排他検証が、生成した引数列に対して「同時に指定できない組み合わせを受理しない」こと (`issues/0053` が単体で広げようとしている分岐を、プロパティとして一般化できるか検討する)
...
前提として `issues/0071` (lib ターゲット追加) が必要。`CODEBASE.md` の規約 (バージョン 2026.0.0 の間は develop で開発しブランチを切らない) により develop へ直接コミットする。`Branch:` は名目上の名前であり実ブランチは切らない (`issues/0057` と同じ運用)。
