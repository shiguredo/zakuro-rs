...
4. tombi の lint / format フックを追加する。対象は TOML ファイルとし、自動生成ファイルである `Cargo.lock` は除外する。書き方は `shiguredo-rust` スキル同梱の参考設定の tombi 節に倣い、外部 repo は rev をタグで固定して追加する (参考設定は `tombi-toml/tombi-pre-commit` の `v1.2.0` で、upstream にはより新しいタグがあるので導入時に最新版のタグへ更新し、その版で 4 件の TOML を通し直す)。参考設定の tombi フックは `stages` 未指定なので、方針 2 と同じ理由で `stages = ["pre-commit"]` を明示する (`default_stages` を入れないため)
